...
{
    // 08.13 안건준 추가 - 군단장 잠재능력 슬롯 1건(옵션 종류+등급+확정값).
    ...
    public sealed class CommanderPotentialSlotData
    {
        ...
        [SerializeField] private bool locked; // 추후 재설정 기능에서 사용할 잠금 여부(지금은 항상 false)
        ...
        // 08.13 안건준 추가 - "잠재 능력 변경" 재추첨. 값이 있고 잠겨있지 않은 슬롯만 다른 옵션으로 교체할 수 있다.
        ...
        // 08.13 안건준 추가 - "옵션 스탯 변경". 옵션 종류·등급은 그대로 두고 수치만 같은 등급 범위 안에서
        // 다시 뽑는다. 잠금은 "잠재 능력 변경"(옵션 자체가 바뀌는 것)만 막는 용도라 여기서는 잠금 여부를 보지 않는다.
        ...
        // 08.13 안건준 추가 - 사용자가 자물쇠 아이콘으로 재추첨 대상에서 제외할 슬롯을 잠그거나 푼다.
    ...
    // 08.13 안건준 추가 - 군단장 잠재능력 5슬롯 저장 데이터. 지금은 잠재능력 레벨(해금) 시스템이
    // 없어서 1번 슬롯만 채워지지만, 추후 해금 단계가 오르면 순서대로 슬롯이 채워질 것을 감안해
    // 구조 자체는 5개로 미리 준비해둔다.
    [Serializable]
    public sealed class CommanderPotentialData
    {
        ...
        // 08.13 안건준 추가 - "수호자의 힘" 해금 진행도. 1단계에서 시작해 100 경험치마다 다음 단계로 올라가고,
        // 단계 수만큼 잠재능력 슬롯이 순서대로 해금된다(1단계=1슬롯 ... 5단계=5슬롯 전부).
        public const int MaxStage = SlotCount;
        public const long ExperiencePerStage = 100;
        public const long ExperiencePerReroll = 10; // "잠재 능력 변경" 버튼 1회 클릭당 증가량

        // 08.13 안건준 추가 - "능력 잠금" 강화석 비용. 이미 잠긴 슬롯 수(0~4)를 기준으로 다음 슬롯을
        ...
        // 08.13 안건준 추가 - "잠재 능력 변경" 버튼 클릭 시 "수호자의 힘" 경험치를 올리고,
        // 100을 채우면 다음 단계로 승급하며 잠재능력 슬롯 해금 수(UnlockedSlotCount)도 함께 늘어난다.
        ...
        // 08.13 안건준 추가 - 값이 있고 잠기지 않은 슬롯을 다른 옵션으로 교체("잠재 능력 변경").
        ...
        // 08.13 안건준 추가 - 재추첨 대상에서 제외할 슬롯 잠금/해제 토글.
        ...
        // 08.13 안건준 추가 - 현재 잠긴 슬롯 수(다음 잠금 비용 계산용).
        ...
        // 08.13 안건준 추가 - "옵션 스탯 변경"(잠금 여부와 무관하게 값만 다시 뽑음).
    ...
    // 08.13 안건준 추가 - "잠재 능력 변경" 1회 요청에서 슬롯 1개에 새로 뽑힌 결과(재추첨은 슬롯이 여러 개 해금된
    // 이후에도 한 번에 적용될 수 있어 리스트로 주고받는다). GameProgressChange가 이 값을 그대로 저장해 두었다가
    // TryApply에서 결정론적으로 반영한다(추첨 자체는 이 구조체를 만들기 전에 이미 끝나 있어야 한다).
    ...
    public readonly struct CommanderPotentialView
    {
        ...
        // 08.13 안건준 추가 - "수호자의 힘" 단계(1~5)와 현재 단계 안의 경험치(0~100).
