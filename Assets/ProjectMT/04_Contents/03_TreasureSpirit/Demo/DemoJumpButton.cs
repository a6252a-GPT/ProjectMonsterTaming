...
namespace ProjectMT.Contents.TreasureSpirit.Demo
{
    ...
    public sealed class DemoJumpButton : MonoBehaviour
    {
            ...
            Sprite iceIcon = null)
        {
            ...
            {
                authored.RebindAuthored(hudRoot, playerMove, iceIcon);
                return authored;
            }
            DemoJumpButton runtimeView = FindExisting(hudRoot);
            if (runtimeView != null)
            {
                runtimeView.Rebind(playerMove, iceIcon);
                return runtimeView;
            }
            ...
            Sprite iceIcon)
        {
            ...
            jumpButton = button;
            jumpFill = fill;
            jumpCanvasGroup = canvasGroup;
            mapButton = authoredMapButton;
            mapRim = null;
            mapLabel = null;
            ...
            if (jumpButton != null)
            {
                ...
                jumpButton.onClick.AddListener(HandleJumpClicked);
            }

            if (mapButton != null)
            {
                mapButton.onClick.RemoveListener(ToggleAutomap);
                mapButton.onClick.AddListener(ToggleAutomap);
            }

            Transform attackRoot = hudRoot.Find(AttackObjectName);
            ActionButton attack = attackRoot != null
                ? ResolveActionButton(attackRoot)
                : CreateActionButton(
                    hudRoot,
                    AttackObjectName,
                    ResolveAuthoredAttackPosition(transform as RectTransform),
                    IceSize,
                    IceRim,
                    PlateColor,
                    iceIcon != null ? iceIcon : DemoHudArt.Ice,
                    8f,
                    ResolveHudFont(hudRoot),
                    "얼음",
                    Color.white,
                    true);
            ...
            attackCanvasGroup = attack.Group;
            if (attackIcon != null)
            {
                attackIcon.sprite = iceIcon != null ? iceIcon : DemoHudArt.Ice;
            }
        ...
        private void Refresh()
        {
            ...
            if (authoredPresentation)
            {
                if (jumpFill != null)
                {
                    jumpFill.fillAmount = jumpFillAmount;
                }
            }
            else
            {
            ...
            if (mapButton != null)
            {
                mapButton.interactable = visible;
            }

            if (!authoredPresentation && mapRim != null)
            {
                mapRim.color = mapOpen ? MapRimOpen : MapRim;
            }

            if (!authoredPresentation && mapLabel != null)
            {
        ...
        private static Vector2 ResolveAuthoredAttackPosition(RectTransform jumpRect)
        {
            if (jumpRect == null)
            {
                return IcePosition;
            ...
            Vector2 size = jumpRect.rect.size;
            if (size.x <= 0f || size.y <= 0f)
            {
                size = jumpRect.sizeDelta;
            }

            Vector2 center = jumpRect.anchoredPosition + Vector2.Scale(
                new Vector2(0.5f - jumpRect.pivot.x, 0.5f - jumpRect.pivot.y),
                size);
            return new Vector2(
                center.x - (size.x * 0.5f) - ButtonGap - (IceSize * 0.5f),
                center.y);
        }

        private static ActionButton ResolveActionButton(Transform root)
        {
            Transform fillTransform = root.Find("CooldownFill");
            Transform iconTransform = root.Find("Plate/Icon");
