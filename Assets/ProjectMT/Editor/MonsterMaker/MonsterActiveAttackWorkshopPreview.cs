    ...
    internal sealed class MonsterActiveAttackWorkshopPreview : IDisposable // 기본공격 조립소형 독립 판정 Preview
    {
        ...
        private void BuildTimeline(int onlyStepIndex)
        {
            ...
            for (var index = first; index <= last; index++)
            {
                ...
                var targetIndex = ResolveTargetIndex(step.TargetPolicy, previousTarget);
                ...
                var travel = step.IsProjectile
                    ? Vector3.Distance(Vector3.zero, targetPositions[targetIndex]) / step.ProjectileSpeed
                    : 0f;
                var impactAt = launchAt + travel;
                var endAt = impactAt + Mathf.Max(0.3f, step.ProgressionDuration, step.VisualDuration * 0.5f);
                timeline.Add(new PreviewStep(index, step, targetIndex, startAt, launchAt, impactAt, endAt));
                time = endAt;
            }
        }

        private int ResolveTargetIndex(MonsterActiveTargetPolicy policy, int previous)
        {
            return 0; // 기본공격 조립소와 동일하게 표준 대상 1명만 표시
        }

        private void ShowStep(PreviewStep item)
        {
            if (item.TargetIndex < 0 || item.TargetIndex >= targetPositions.Count) return;
            var target = targetPositions[item.TargetIndex];
        ...
        private void UpdateDelivery(float elapsed, int activeIndex)
        {
            ...
            delivery.transform.localPosition = Vector3.Lerp(origin, targetPositions[item.TargetIndex], ratio) +
                                               Vector3.up * 0.25f;
        ...
        {
            public PreviewStep(int sourceIndex, MonsterActiveAttackStep step, int targetIndex,
