...
namespace ProjectMT.EditorTools.MonsterMaker
{
    ...
    public sealed partial class MonsterBasicAttackWorkshopWindow : EditorWindow // V1 Legacy UI와 V2 공용 제작 코어
    {
        ...
        [MenuItem("JC Tool/Monster/Legacy/기본공격 조립소 V1")]
        ...
        public static void Open(MonsterMakerDraft draft)
        {
            ...
            {
                if ((staleWindow.hideFlags & HideFlags.HideAndDontSave) != 0) continue; // V2의 비가시 제작 코어는 Legacy 창으로 취급하지 않는다.
                if (!staleWindow.TryResolveUnsavedChanges("다른 기본공격 조립소를 열기"))
                {
                    staleWindow.Focus();
                    return;
                }
        ...
        private void DrawAssembler()
        {
                       ...
                       GUILayout.Width(AssemblerWidth)))
            {
                ...
                {
                    var previousVfxContractSignature = recipe.BuildVfxContractSignature();
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        if (!string.Equals(
                                previousVfxContractSignature,
                                recipe.BuildVfxContractSignature(),
                                StringComparison.Ordinal))
                        {
                            recipe.vfxSlots = MonsterBasicAttackVfxContractTemplates.Reconcile(
                                workingProfile,
                                recipe.vfxSlots,
                                out var reconcileResult);
                            CompileWorkingProfile();
                            message =
                                $"공격 방식에 맞춰 VFX 공간을 정리했습니다. 유지 {reconcileResult.Retained} · 추가 {reconcileResult.Added} · 제외 {reconcileResult.Archived}";
                                     ...
                                     StringComparison.OrdinalIgnoreCase)) > 1)
                    {
                        EditorGUILayout.HelpBox(
                            ...
                            MessageType.Error);
                             ...
                             !MonsterBasicAttackVfxCompatibility.TryValidateSlot(
                                 workingProfile,
                                 slot.Compile(),
                                 out var compatibilityError))
                    {
                        EditorGUILayout.HelpBox(
                            $"현재 공격 방식에서는 발생할 수 없는 계약입니다.\n{compatibilityError}",
                            MessageType.Error);
                    }
                }
            }
        ...
        private void StartBlank()
        {
            ...
            messageType = MessageType.Info;
            CompileWorkingProfile();
            recipe.vfxSlots = MonsterBasicAttackVfxContractTemplates
                .Build(workingProfile)
                .Select(BasicAttackWorkshopVfxSlot.From)
                .ToList();
            CompileWorkingProfile();
            Repaint();
        ...
        {
            if (!TryResolveUnsavedChanges("빈 기본공격을 시작하기")) return;
            StartBlank();
        }

        private void TryLoadProfile(MonsterBasicAttackProfile target)
        {
            if (target == null || target == loadedProfile) return;
            if (!TryResolveUnsavedChanges($"[{target.AttackId}] {target.DisplayName} 프리셋을 불러오기")) return;
            LoadProfile(target);
        }
        ...
        private void SetWorkCopyDirty(bool value)
        {
            ...
            saveChangesMessage = "기본공격 조립소의 변경 사항을 저장하시겠습니까?";
        }
        ...
        private void RebuildPreview()
        {
            ...
            {
                MonsterWorkshopPreviewSceneRecovery.UnregisterOwner(previewUtility);
            ...
            if (previewUtility == null)
            {
                ...
                MonsterWorkshopPreviewSceneRecovery.RegisterOwner(previewUtility);
            }
